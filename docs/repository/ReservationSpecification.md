...
## Explanation
- **filter(...)**: Returns a JPA Specification for filtering reservations by status, branch, table, customer, reservation time range, and party size range. Each filter is applied only if the parameter is not null or empty.