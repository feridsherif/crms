...
public class InventoryService {
	...
	public InventoryItemResponseDTO adjustStock(Long id, StockAdjustmentDTO dto) {
		InventoryItem item = inventoryItemRepository.findById(id)
				.orElseThrow(() -> new com.hilcoe.crms.exception.InventoryItemNotFoundException(
		...
		// Notify chef/manager about stock adjustment
		try {
			String dataJson = new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(updated);
			notificationService.sendToRole("CHEF", "Stock Adjusted",
					"Stock for item '" + updated.getName() + "' was adjusted. New quantity: " + newQuantity, dataJson);
			notificationService.sendToRole("MANAGER", "Stock Adjusted",
					"Stock for item '" + updated.getName() + "' was adjusted. New quantity: " + newQuantity, dataJson);
			if (newQuantity.compareTo(updated.getThreshold()) < 0) {
				notificationService.sendToRole("CHEF", "Low Stock Alert",
						"Stock for item '" + updated.getName() + "' is below threshold!", dataJson);
				notificationService.sendToRole("MANAGER", "Low Stock Alert",
						"Stock for item '" + updated.getName() + "' is below threshold!", dataJson);
			}
		} catch (Exception e) {
			/* log or ignore */ }
	...
	public InventoryReportDTO generateReport() {
		...
		for (InventoryItem item : items) {
			...
			List<StockMovement> movements = stockMovementRepository
					.findTop5ByInventoryItem_InventoryItemIdOrderByMovementIdDesc(item.getInventoryItemId());
			...
			for (StockMovement m : movements) {
				...
				// No date field in StockMovement, so use movementId as proxy (or add date if
				// available)
	...
	public PaginatedResponseDTO<InventoryItemResponseDTO> getInventoryPaginated(Pageable pageable) {
		...
		List<InventoryItemResponseDTO> content = page.getContent().stream().map(this::toResponseDTO)
	...
	public PaginatedResponseDTO<InventoryItemResponseDTO> getInventoryPaginatedAdvanced(String name, Long supplierId,
			String unit, java.math.BigDecimal minQuantity, java.math.BigDecimal maxQuantity,
			java.math.BigDecimal minThreshold, java.math.BigDecimal maxThreshold, int page, int size, String sortBy,
			String direction) {
		Sort sort = Sort.by(
				direction != null && direction.equalsIgnoreCase("desc") ? Sort.Direction.DESC : Sort.Direction.ASC,
				(sortBy != null && !sortBy.isEmpty()) ? sortBy : "inventoryItemId");
		Pageable pageable = PageRequest.of(page, size, sort);
		var spec = InventoryItemSpecification.filter(name, supplierId, unit, minQuantity, maxQuantity, minThreshold,
				maxThreshold);
		Page<InventoryItem> result = inventoryItemRepository.findAll(spec, pageable);
		List<InventoryItemResponseDTO> content = result.getContent().stream().map(this::toResponseDTO)
				.collect(Collectors.toList());
